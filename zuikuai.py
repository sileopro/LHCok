...
def fetch_and_parse_lottery_data(lottery_type):
    """依次尝试主API和所有备用API，只有获取并解析都成功才返回结果"""
    api_config = API_CONFIGS[lottery_type]
    apis = [api_config['main']] + api_config['backup']
    for idx, api_url in enumerate(apis):
        if idx == 0:
            logger.info(f"尝试从主API获取{LOTTERY_NAMES[lottery_type]}数据: {api_url}")
        else:
            logger.info(f"尝试从备用API获取{LOTTERY_NAMES[lottery_type]}数据: {api_url}")
        try:
            ...
            if response.status_code == 200 and response.text:
                ...
                if lottery_info:
                    ...
                    logger.error(f"解析{LOTTERY_NAMES[lottery_type]}数据失败 (API: {api_url})")
            else:
                logger.error(f"API请求失败: {api_url}")
        except Exception as e:
            logger.error(f"API请求异常: {api_url} 错误: {str(e)}")
    logger.error(f"所有API获取或解析{LOTTERY_NAMES[lottery_type]}数据均失败")
    return None
...
def get_lottery_results():
    ...
    for lottery_type in lottery_types:
        ...
        # 获取并解析API数据
        lottery_info = fetch_and_parse_lottery_data(lottery_type)
        if not lottery_info:
            logger.error(f"❌ 获取或解析{LOTTERY_NAMES[lottery_type]}数据失败")
            continue
